...
public class PatchedEntityRenderer extends EntityRenderer {

    // the method we're patching
    private void orientCamera(float partialTicks) {
        Entity entity = mc.getRenderViewEntity();
        float eyeHeight = entity.getEyeHeight();
        double headX = entity.prevPosX + (entity.posX - entity.prevPosX) * (double) partialTicks;
        double headY = entity.prevPosY + (entity.posY - entity.prevPosY) * (double) partialTicks + (double) eyeHeight;
        double headZ = entity.prevPosZ + (entity.posZ - entity.prevPosZ) * (double) partialTicks;
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            double thirdPersonDistance = 4.0;

            if (mc.gameSettings.debugCamEnable) {
                GlStateManager.translate(0.0F, 0.0F, (float) (-thirdPersonDistance));
            } else {
                ...
                if (FarF5Mod.instance.config.modEnabled) {

                    thirdPersonDistance = FarF5Mod.instance.config.farDistance;

                } else {

                    double d4 = (double) (-MathHelper.sin(f1 * 0.017453292F) * MathHelper.cos(f2 * 0.017453292F)) * thirdPersonDistance;
                    double d5 = (double) (MathHelper.cos(f1 * 0.017453292F) * MathHelper.cos(f2 * 0.017453292F)) * thirdPersonDistance;
                    double d6 = (double) (-MathHelper.sin(f2 * 0.017453292F)) * thirdPersonDistance;
                    ...
                    for (int i = 0; i < 8; ++i) {
                        ...
                        f4 = f4 * 0.1F;
                        f5 = f5 * 0.1F;
                        RayTraceResult raytraceresult = mc.world.rayTraceBlocks(new Vec3d(headX + (double) f3, headY + (double) f4, headZ + (double) f5), new Vec3d(headX - d4 + (double) f3 + (double) f5, headY - d6 + (double) f4, headZ - d5 + (double) f5));

                        if (raytraceresult != null) {
                            double d7 = raytraceresult.hitVec.distanceTo(new Vec3d(headX, headY, headZ));

                            if (d7 < thirdPersonDistance) {
                                thirdPersonDistance = d7;
                            }
                        }
                    }
                }
        ...
        // ignore cloudFog, it's always false in 1.10
//        headX = entity.prevPosX + (entity.posX - entity.prevPosX) * (double) partialTicks;
//        headY = entity.prevPosY + (entity.posY - entity.prevPosY) * (double) partialTicks + (double) eyeHeight;
//        headZ = entity.prevPosZ + (entity.posZ - entity.prevPosZ) * (double) partialTicks;
//        boolean myCloudFog = mc.renderGlobal.hasCloudFog(headX, headY, headZ, partialTicks);
//        ObfuscationReflectionHelper.setPrivateValue(EntityRenderer.class, this, myCloudFog, "cloudFog");
    }

    // all the remaining code is copied from EntityRenderer
    ...
    public void loadEntityShader(Entity entityIn) {
        if (OpenGlHelper.shadersSupported) {
            ...
            } else ForgeHooksClient.loadEntityShader(entityIn, this);
        }
    }
    ...
    private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            return ForgeHooksClient.getFOVModifier(this, entity, iblockstate, partialTicks, f);
        }
    }
    ...
    private void renderHand(float partialTicks, int pass) {
        if (!this.debugView) {
            ...
            if (!ForgeHooksClient.renderFirstPersonHand(mc.renderGlobal, partialTicks, pass))
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ForgeHooksClient.drawScreen(this.mc.currentScreen, k1, l1, partialTicks);
                    ...
                    crashreportcategory.setDetail("Screen size", new ICrashReportDetail<String>() {
                        public String call() throws Exception {
                            return String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", new Object[]{Integer.valueOf(scaledresolution.getScaledWidth()), Integer.valueOf(scaledresolution.getScaledHeight()), Integer.valueOf(mc.displayWidth), Integer.valueOf(mc.displayHeight), Integer.valueOf(scaledresolution.getScaleFactor())});
                        }
                    });
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        if (flag && this.mc.objectMouseOver != null && !entity.isInsideOfMaterial(Material.WATER)) {
            ...
            if (!ForgeHooksClient.onDrawBlockHighlight(renderglobal, entityplayer, mc.objectMouseOver, 0, partialTicks))
    ...
    private void setupFog(int startCoords, float partialTicks) {
        ...
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        IBlockState iblockstate = ActiveRenderInfo.getBlockStateAtEntityViewpoint(this.mc.world, entity, partialTicks);
        float hook = ForgeHooksClient.getFogDensity(this, entity, iblockstate, partialTicks, 0.1F);
        if (hook >= 0) GlStateManager.setFogDensity(hook);
            ...
            ForgeHooksClient.onFogRender(this, entity, iblockstate, partialTicks, startCoords, f);
        }
